...
\label{prop:cost}
Let $\rho = \ell/L$ and $D = w + (1-w)\gamma$. Under
Definitions~\ref{def:cost} and \ref{def:headfrac}, one round with lookahead
$\gamma$ costs
\begin{align}
  C_{\mathrm{self}}(\rho,\gamma,w)
    &= \gamma\big[(1-u)\rho + u\big]
     \;+\; \big[(1-u)(1-\rho) + u\big]\,D ,
    \label{eq:cself}\\
  C_{\mathrm{ext}}(c_d,\gamma,w)
    &= \gamma c_d \;+\; D ,
    \label{eq:cext}
\end{align}
for early-exit self-drafting at layer $\ell$ and an external draft model of
relative per-position cost $c_d$ respectively.
\end{proposition}

\begin{proof}
Each of the $\gamma$ draft steps evaluates blocks $1..\ell$ at one position,
costing $(1-u)\rho$ by Definitions~\ref{def:cost} and~\ref{def:headfrac}, plus
the head, costing $u$; at $m=1$ these are independent of $w$. Verification
evaluates $M$ at the $\gamma$ drafted positions. Blocks $1..\ell$ at those
positions were already evaluated during drafting and their cached keys and
values are exactly those verification requires, so only blocks $\ell+1..L$ are
evaluated, costing $(1-u)(1-\rho)D$, together with the head at $\gamma$
positions, costing $uD$. The external draft shares no blocks with $M$ and pays
full verification $D$.
\end{proof}

\begin{theorem}[Self-drafting dominates in every regime]
\label{thm:dominance}
...
strictly greater speedup.
\end{theorem}

\begin{proof}
From \eqref{eq:cself}--\eqref{eq:cext},
\begin{equation}
  C_{\mathrm{ext}} - C_{\mathrm{self}}
  = \gamma\big[c_d - u - (1-u)\rho\big] \;+\; (1-u)\rho\, D ,
\end{equation}
whose first term is non-negative by hypothesis and whose second is strictly
positive. Hence $C_{\mathrm{self}} < C_{\mathrm{ext}}$, while
$\E[Y\mid\alpha_\ell] \ge \E[Y\mid\alpha_d] > 0$ by Proposition~\ref{prop:yield}
and monotonicity of the yield in the acceptance events.
\end{proof}
...
\begin{remark}[On the value of cache reuse]
\label{rem:kvreuse}
It is sometimes objected that reusing the draft's cached keys and values is
worthless in the memory-bound regime, on the grounds that the saving is in
per-position arithmetic, which $w = 1$ already prices at zero. That is not what
is being saved. Reuse allows verification to \emph{skip blocks $1..\ell$
entirely}, and therefore to skip streaming their parameters --- which is exactly
the cost that $w = 1$ charges. This is the term $(1-u)\rho D$ in
Theorem~\ref{thm:dominance}, and it is why self-drafting wins even at
$c_d = u + (1-u)\rho$, where the two schemes pay identical drafting cost. Two
caveats are real: the saving applies only to positions on the accepted prefix,
and ``identical'' holds up to floating-point reduction order between the two
kernel shapes.
\end{remark}
