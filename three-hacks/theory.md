...
| exp7 on a peaked random HMM printed "DISPROOF: Thm 5.8 is practically empty" | **Unearned, and the cyclic HMM reverses it.** Peaked rows collapse the belief so no mixing is needed. On the cyclic HMM (belief entropy 0.92/1.39) dense-1 captures 94% of headroom, diag-1 44%, diag-3 53%, and 4× width only 62% — a tenfold excess-NLL gap that neither depth nor width closes. **Thm 5.8 has bite, exactly where its proof (b) says: rotating belief, complex eigenvalues** | §6.9, `exp7 --hmm cyclic` |
| exp6 Part B, first design: `tanh(G(s))` idle dynamics | Every cell at chance including (σ=0, none): 64 tanh steps contract the state to zero. Replaced by identity dynamics, matching Part A | `exp6_partB_tanh_ARTEFACT.log` |
| exp6 Part B, second design: identity dynamics, learned codebook | Reference cell learns (1.00), but **(σ=0, k=1) is at chance** where projection is free — accuracy monotone in the *number* of projections. Optimisation, not error correction. The learned-codebook question is **open**, and the script now says so instead of printing "none wins" | `exp6_partB_unguarded.log`, §6.8 |
| Thesis 1 dead at Qwen's head fraction (`S = 1.000`, degenerate `ρ=1`) | **Rescued by training at zero quality cost.** Early-stopped auxiliary early-exit loss: `S = 1.044` at `u=0.26` with final-layer val loss *improving* 0.03; `1.072` if you accept +0.08. Tail energy falls at every layer — Thm 3.7's mechanism. A 4% margin on an inflated-acceptance model is the predicted sign, not a result to build on | §6.6 |
